...
BUTTON_ACTIVE_LOW = True   # 按下时电平为低
...
def init_gpio():
    ...
    for pin in GPIO_BUTTONS:
        ...
        try:
            ...
            print(f"⚠️ 设置 GPIO {pin} 方向失败：{e}")
        try:
            with open(f"/sys/class/gpio/gpio{pin}/value", "r") as f:
                GPIO_LAST_STATE[pin] = int(f.read().strip())
            print(f"✅ GPIO {pin} 已就绪，当前电平：{GPIO_LAST_STATE[pin]}")
        except Exception as e:
            print(f"⚠️ 读取 GPIO {pin} 初始电平失败：{e}")
...
def main():
    ...
    print("📷 AI 相机 (调试版) | Q 退出 | S 拍照 | L 相册 | G 螺旋")
    ...
    while True:
        ...
        if gpio_key is not None:
            print(f"✅ GPIO 事件捕获，功能键：{gpio_key}")   # 调试打印
            key = ord(gpio_key)
